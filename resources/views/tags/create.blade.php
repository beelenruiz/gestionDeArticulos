<x-app-layout>
    <x-self.base>
        ...
        <h1 class="text-xl text-center text-slate-700 font-bold mb-5"> NUEVA ETIQUETA </h1>
        ...
        @csrf
            ...
            <div class="mb-6">
                ...
                <textarea 
                    id="description" name="description" rows="3" class="w-full p-2 text-medium rounded-lg focus:ring-slate-700 focus:border-slate-700" placeholder="Escribe una descripción...">{{old('description')}}</textarea>
                <x-input-error for="description" />
            </div>

            <div class="mb-6">
                <label for="color" class="block text-medium font-medium text-gray-700 mb-2">Etiqueta</label>
                <input value="{{old('color')}}" type="color" name="color" id="color" class="w-full p-1 h-10 text-medium rounded-lg focus:ring-slate-700 border-slate-700"/>
